...
export default function SoilTempChart({ data, thresholds = [] }) {
  ...
  const minV = Math.min(...values, ...thresholdValues) - 3
  const maxV = Math.max(...values, ...thresholdValues) + 3
      ...
      {thresholds.map((t) => (
        <g key={t.label}>
          <line
            ...
            x2={WIDTH - PAD_R}
            y2={y(t.value)}
            stroke={t.color || 'var(--accent-2)'}
            strokeDasharray="4 4"
            strokeWidth="1.5"
          />
          <text x={WIDTH - PAD_R} y={y(t.value) - 5} fill={t.color || 'var(--accent-2)'} fontSize="10" textAnchor="end">
            {t.label}
          </text>
        </g>
      ))}
